...
describe('resolvePlayback in real time', () => {
  ...
  it('drops the listener mid-broadcast rather than starting at track one', () => {
    ...
    const p = resolvePlayback(dayparted, clock.read(at('2026-03-04T06:04:10Z')), tracks);
  ...
  it('loops the playlist for the length of the program', () => {
    const p = resolvePlayback(dayparted, clock.read(at('2026-03-04T06:05:10Z')), tracks);
  ...
  it('reports when the current track hands over', () => {
    ...
    const p = resolvePlayback(dayparted, clock.read(now), tracks);
  ...
  it('is stable: two readings a second apart advance by exactly a second', () => {
    ...
    const a = resolvePlayback(dayparted, clock.read(t), tracks)!;
    const b = resolvePlayback(dayparted, clock.read(t + 1000), tracks)!;
...
describe('resolvePlayback in game time', () => {
  ...
  it('runs the dayparts fast while the music stays at 1x', () => {
    ...
    expect(resolvePlayback(dayparted, later, tracks)?.offsetSec).toBeCloseTo(30, 3);
  });

  it('compresses the music too when asked', () => {
    const later = clock.read(GAME_EPOCH_MS + 6 * 60_000 + 3_000);
    const p = resolvePlayback(dayparted, later, tracks, { timelineScale: later.timelineRate });
